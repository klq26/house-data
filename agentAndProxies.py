...
class GetIpProxy():
    ...
    # 获得IP代理池子，结果以 key-value 形式存入 self.infos
    def getIpPool(self):
        ...
        for item in result:
            ...
            if protocal is not None and protocal != 'socks4/5':
                # host为地址  port为协议内容
                proxy_url = '{0}://{1}:{2}'.format(protocal, json_source.get('host'), json_source.get('port'))
                self.infos[proxy_url.lower()] = protocal.lower()
    
    # 判断某个代理 IP + Port 用来访问目标地址时，是否能成功（例如 status code == 200）
    def judge_ip(self, proxy_url, protocol):
        # 判断给出的代理 ip 是否可用
        http_url = 'http://bj.lianjia.com/'

        print("proxy_url", proxy_url, "http_url", http_url)
        try:
            proxy_dict = {
                protocol: proxy_url
            }
            ...
            print(u"[没有返回]代理{0} 端口号及 IP 不可用".format(proxy_url))
            return False
        else:
            code = response.status_code
            if code >= 200 or code < 300:
                print(u"代理 {0} 端口号及 IP 可用".format(proxy_url))
                return True
            else:
                print(u"[有返回，但是状态码异常]代理 {0} 端口号及 IP 不可用".format(proxy_url))
                return False

    # 随机从库中抽取一个 IP 代理地址并返回
    def get_random_ip(self):
        for i in range(len(self.infos.keys())):
            judge_ip_status = self.judge_ip(list(self.infos.keys())[i], list(self.infos.values())[i])
            if judge_ip_status:
                return list(self.infos.items())[i]

    # 通过动态代理 IP 访问 url。（如果当前代理服务器地址为空或不可返回 200，会自动递归调用自己，换下一个 IP，直至成功）
